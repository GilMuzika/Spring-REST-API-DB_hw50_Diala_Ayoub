...
public class ClassRoomRepository implements IClassRoomRepository{
    ...
    public List<Student> getByExternalClass() {
        String query =
        String.format("SELECT * FROM %s WHERE class_id IN (SELECT id FROM %s WHERE classroomtype = 'EXTERNAL');"
        ,StudentRepository.getSTUDENT_TABLE_NAME(),CLASSROOM_TABLE_NAME);
